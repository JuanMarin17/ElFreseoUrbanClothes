...
const api = {
  getByProduct: (productId) =>
    fetch(`${BASE_URL}/product/${productId}`, { headers: authHeaders() }).then(
      handleRes,
    ),
  ...
  getReplies: (reviewId) =>
    fetch(`${BASE_URL}/${reviewId}/replies`, { headers: authHeaders() }).then(
      handleRes,
    ),
...
function Toast({ message, type, onClose }) {
  ...
  return (
    <div className={`rvx-toast rvx-toast--${type}`}>
      ...
      <span>{message}</span>
      <button onClick={onClose} className="rvx-toast-close">
        <X size={13} />
      </button>
    </div>
  );
...
function ReviewForm({ productId, onCreated, onToast, onClose }) {
  ...
  const submit = async (e) => {
    e.preventDefault();
    if (rating === 0) {
      onToast("Selecciona al menos 1 estrella", "error");
      return;
    }
    try {
      setLoading(true);
    ...
    <div className="rvx-form-overlay" onClick={onClose}>
      <div className="rvx-form-modal" onClick={(e) => e.stopPropagation()}>
        <div className="rvx-form-header">
          ...
          <button className="rvx-icon-btn" onClick={onClose}>
            <X size={17} />
          </button>
        </div>
        ...
        <form onSubmit={submit} className="rvx-form-body">
          ...
          <div className="rvx-form-actions">
            ...
            <button
              type="submit"
              className="rvx-btn-primary"
              disabled={loading}
            >
              {loading ? (
                <Loader size={14} className="rvx-spin" />
              ) : (
                <Plus size={14} />
              )}
              Publicar reseña
...
function RepliesSection({ reviewId, currentUserId, isStoreOwner, onToast }) {
  ...
  return (
    <div className="rvx-replies">
      {loading && (
        <div className="rvx-replies-loading">
          <Loader size={13} className="rvx-spin" /> Cargando...
        </div>
      )}
...
function ReviewCard({
  ...
  const handleDelete = async () => {
    if (
      !window.confirm("¿Eliminar tu reseña? Esta acción no se puede deshacer.")
    )
      return;
    try {
      setDeleting(true);
  ...
  const handleReact = async (type) => {
    try {
      ...
      onToast(
        type === "LIKE" ? "👍 ¡Gracias por tu reacción!" : "👎 Recibido",
        "success",
      );
    } catch (err) {
      onToast(err.message, "error");
  ...
  return (
    <div className="rvx-summary">
      ...
      <div className="rvx-summary-score">
        ...
        <span className="rvx-summary-total">
        ...
        {[5, 4, 3, 2, 1].map((star) => {
          ...
          return (
            <div key={star} className="rvx-bar-row">
              <span className="rvx-bar-label">
                {star}
                <Star size={10} className="rvx-star--fill" />
              </span>
...
export function MyReviewsPage({ currentUserId }) {
  ...
  const onDelete = (reviewId) =>
    setReviews((p) => p.filter((r) => r.reviewId !== reviewId));
    ...
    <section className="rvx-root">
      ...
      {!loading && reviews.length === 0 && (
        <div className="rvx-empty">
          ...
          <p className="rvx-empty-sub">
            Cuando compres productos podrás dejar tu opinión.
          </p>
        </div>
      )}
