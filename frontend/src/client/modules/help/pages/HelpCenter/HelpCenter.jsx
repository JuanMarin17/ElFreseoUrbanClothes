...
/**
 * Simula obtener el usuario autenticado desde tu sistema de auth.
 * Reemplaza esto con tu contexto real (AuthContext, Redux, localStorage, etc.)
 * El objeto debe tener: { id, email, role }
 * role puede ser "OWNER" (admin) o cualquier otro string (usuario normal)
 */
const useCurrentUser = () => {
  // ↓ Cambia esto por tu hook/contexto de autenticación real
  return {
    id:    'user-uuid-aqui',
    email: 'usuario@email.com',
    role:  'USER', // 'OWNER' para el admin
  };
};
...
const api = {
  ...
  // Mis tickets (usuario)
  getMyTickets: (user) =>
    fetch(`${API_BASE}/support/tickets/me`, {
      headers: buildHeaders(user),
    }).then(handleResponse),
  ...
  getTicket: (user, ticketId) =>
    fetch(`${API_BASE}/support/tickets/${ticketId}`, {
      headers: buildHeaders(user),
    }).then(handleResponse),

  // Mensajes de un ticket
  getMessages: (user, ticketId) =>
    fetch(`${API_BASE}/support/tickets/${ticketId}/messages`, {
      headers: buildHeaders(user),
    }).then(handleResponse),
  ...
  closeTicket: (user, ticketId) =>
    fetch(`${API_BASE}/support/tickets/${ticketId}/close`, {
      method:  'PATCH',
      headers: buildHeaders(user),
    }).then(handleResponse),
};
...
function Toast({ message, type, onClose }) {
  ...
  const bg = type === 'error' ? '#ef4444' : 'var(--fire)';
  return (
    <div style={{
      position: 'fixed', bottom: 24, right: 24, zIndex: 9999,
      background: bg, color: '#fff', borderRadius: 8,
      padding: '12px 20px', fontSize: '.88rem', fontWeight: 600,
      display: 'flex', alignItems: 'center', gap: 10,
      boxShadow: `0 4px 20px ${bg}50`,
      maxWidth: 340,
    }}>
...
function TicketConversation({ ticket, user, onBack, onToast, onTicketUpdated }) {
  ...
  const [sending, setSending]     = useState('');  // 'reply' | 'close' | ''
  const isOwner = user.role === 'OWNER';

  const loadMessages = useCallback(async () => {
    try {
      setLoading(true);
      const data = await api.getMessages(user, ticket.ticketId);
      setMessages(data);
    } catch (err) {
  ...
  const handleReply = async () => {
    ...
    try {
      ...
      setReply('');
      onToast('Respuesta enviada. El usuario recibirá un email.');
      onTicketUpdated();
      loadMessages();
    } catch (err) {
      onToast(err.message, 'error');
  ...
  const handleClose = async () => {
    ...
    try {
      ...
      onToast('Ticket cerrado correctamente.');
      onTicketUpdated();
      onBack();
    } catch (err) {
      onToast(err.message, 'error');
...
function TicketList({ user, onSelect, onToast, refreshTrigger }) {
  ...
  const loadTickets = useCallback(async () => {
    try {
      setLoading(true);
      const data = isOwner
        ? await api.getAllTickets(user)
        : await api.getMyTickets(user);
      setTickets(data);
    } catch (err) {
      onToast(err.message, 'error');
    } finally {
  ...
  }, [user, isOwner, onToast]);

  useEffect(() => { loadTickets(); }, [loadTickets, refreshTrigger]);
  ...
  return (
    <div className="hc-ticket-list">
      <div className="hc-ticket-list-header">
        ...
        <button className="hc-refresh-btn" onClick={loadTickets} title="Actualizar">
          <RefreshCcw size={15} />
        </button>
      </div>
...
function CreateTicketModal({ user, onClose, onCreated, onToast }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      onToast('Ticket creado. Recibirás un email de confirmación.');
      onCreated();
      onClose();
    } catch (err) {
      onToast(err.message, 'error');
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: '1.5rem', justifyContent: 'flex-end' }}>
          <button
            ...
            style={{
              ...
              color: 'var(--off)', padding: '10px 20px', borderRadius: 6,
...
const HelpCenter = () => {
  ...
  /* ── VISTA: HOME (página principal del centro de ayuda) ── */
  return (
    <>
    <HeaderMarket />          {/* ← agregar aquí */}
...
/* Formulario rápido de contacto que crea un ticket */
