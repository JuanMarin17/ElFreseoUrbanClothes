...
function StatCard({
  ...
  return (
    <div className="rp-stat-card">
      {icon && (
        <i
          className={`ti ti-${icon} rp-stat-icon`}
          style={{ color }}
          aria-hidden="true"
        />
      )}
  ...
  useEffect(() => {
    ...
    function draw(ts) {
      ...
      data.forEach((d, i) => {
        ...
        const lbl =
          String(d[labelKey]).slice(8) || String(d[labelKey]).slice(-2);
...
function LineAreaChart({
  ...
  useEffect(() => {
    ...
    const xP = (i) =>
      pad.l + i * ((W - pad.l - pad.r) / Math.max(data.length - 1, 1));
    ...
    function draw(ts) {
      ...
      const pts = data
        .slice(0, full + 1)
        .map((d, i) => ({ x: xP(i), y: yP(d[valueKey]) }));
      if (full < data.length - 1) {
        pts.push({
          x: xP(full) + (xP(full + 1) - xP(full)) * frac,
          y:
            yP(data[full][valueKey]) +
            (yP(data[full + 1][valueKey]) - yP(data[full][valueKey])) * frac,
        });
      }
...
function DashboardSection({ data, loading, error }) {
  ...
  if (error)
    return (
      <AlertBanner
        type="error"
        message={`Error al cargar el panel: ${error}`}
      />
    );
  if (!data) return null;
...
function StockSection({ data, loading, error }) {
  ...
  return (
    <div className="rp-section">
      ...
      <div className="rp-kpi-row">
        <StatCard
          label="Variantes sanas"
          value={summary.healthyStockVariants}
          color="#22c55e"
          icon="circle-check"
        />
        <StatCard
      ...
      {outOfStockItems?.length > 0 && (
        <div className="rp-card">
          <div className="rp-card-title" style={{ color: "#ef4444" }}>
            <i className="ti ti-circle-x" aria-hidden="true" /> Sin stock (
              ...
              <tbody>
                {outOfStockItems.map((item) => (
                  <tr key={item.sku}>
                    ...
                    <td>
                      <code className="rp-sku">{item.sku}</code>
                    </td>
      ...
      {lowStockItems?.length > 0 && (
        <div className="rp-card">
          <div className="rp-card-title" style={{ color: "#f59e0b" }}>
            <i className="ti ti-alert-triangle" aria-hidden="true" /> Stock bajo
...
function OrdersSection({ data, loading, error, period, onPeriodChange }) {
  ...
  if (error)
    return (
      <AlertBanner type="error" message={`Error al cargar órdenes: ${error}`} />
    );
  ...
  return (
    <div className="rp-section">
      ...
      {!data ? (
        <Loader text="Selecciona un período..." />
      ) : (
        <>
          <div className="rp-kpi-row">
            <StatCard
...
function SalesSection({ data, loading, error, period, onPeriodChange }) {
  ...
  if (error)
    return (
      <AlertBanner type="error" message={`Error al cargar ventas: ${error}`} />
    );
      ...
      {!data ? (
        ...
        <>
          <div className="rp-kpi-row">
            ...
            <StatCard
              label="Esta semana"
              value={data.summary.weekRevenue}
              prefix="$"
              color="#3b82f6"
              icon="trending-up"
            />
